...
function addTransport(id) {
  for (let i in id) {
    ...
    let input = ICRender.getGroup("input-item-pipe")
    let transport = ICRender.getGroup("transport-item-pipe")
    let output = ICRender.getGroup("output-item-pipe")
    
    output.add(addID, -1);
    input.add(addID, -1);
    transport.add(addID, -1);
    
    //ICRender.addGroupFor(BlockID[machineID], ["input-item-pipe", "output-item-pipe", "transport-item-pipe"]);
  }
}

addTransport([BlockID.alloySmelter, BlockID.sagmill, BlockID.simpleAlloySmlter, BlockID.simplePoweredFurnace, BlockID.simplesagmill]);

// Vanilla addTransport([54, 61, 62, 154]);
*/
